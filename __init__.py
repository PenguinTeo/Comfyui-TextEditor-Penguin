# text_on_image.py  (放 custom_nodes 下覆盖旧文件)
...
class PenguinTextOnImage:
    ...
    def apply_text(self, text, image,
        ...
        try:    font = ImageFont.truetype(fpath, font_size)
        ...
        if h_anchor == "center":  dx = anchor_x - bbox_w//2
        elif h_anchor == "right": dx = anchor_x - bbox_w
        else:                     dx = anchor_x            # left

        if v_anchor == "center":  dy = anchor_y - bbox_h//2
        elif v_anchor == "bottom":dy = anchor_y - bbox_h
        else:                     dy = anchor_y            # top
...
NODE_CLASS_MAPPINGS        = {"PenguinTextOnImage": PenguinTextOnImage}
NODE_DISPLAY_NAME_MAPPINGS = {"PenguinTextOnImage": "🐧 TextEdit"}
